def build_prompt(quality, genre, identity=None, ses=None, text=None, implicit_essay=False):
    global essay_prompt

    # Select base instructions based on design type
    if implicit_essay:
        # Vignette-based design: identity/SES embedded in text, not labeled explicitly
        base_instructions = f"""Your task is to write a {quality} short essay from the perspective of an undergraduate student who wrote the following text:

\"{text}\"

IMPORTANT: The essay should read as if written naturally by an actual student, rather than a representation of the entire group the student belongs to.

"""
    else:
        # Label-based design: identity/SES stated explicitly
